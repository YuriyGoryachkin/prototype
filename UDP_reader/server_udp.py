...
class JIM_Server2:
    ...
    def receive_response(self, client, request, data):

        socket = request[1]
        print("{} wrote:".format(client[0]))
        print(data)
...
class Server2(DatagramRequestHandler):
    ...
    def handle(self):
        while True:
            try:
                ...
                log_config.log_server.warning('[Server]: handle error')
                os._exit(0)
            handler2.receive_response(client=self.client_address, request=self.request, data=self.data)
...
if __name__ == '__main__':
    ...
    server2 = UdpServer((host, port), Server2)
    # handler = JIM_Server(server)
    handler2 = JIM_Server2(server2)
